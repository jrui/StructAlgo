...
export class MergeSort<T> {
    ...
    constructor(array: T[] = [], comparatorFn = MergeSort.defaultComparator) {
        this.values = [...array];
     ...
     */
    private static merge(left: any[], right: any[], comparatorFn: (a: any, b: any) => number): any[] {
        const result: any[] = [];
        let leftIndex = 0;
        let rightIndex = 0;
    ...
    static sort(array: any[], comparatorFn = this.defaultComparator): any[] {
        ...
        return MergeSort.merge(
            MergeSort.sort(left, comparatorFn),
            MergeSort.sort(right, comparatorFn),
            comparatorFn
        );
    }
    ...
    sort(): T[] {
        ...
        return this.merge(
            (new MergeSort(left, this.defaultComparator)).sort(),
            (new MergeSort(right, this.defaultComparator)).sort()
        );
    }
}
